...
HitRecord Sphere::findClosestIntersection(const glm::vec3 &rayOrigin, const glm::vec3 &rayDirection)
{
	...
	// Page 77
	float discriminant = pow(glm::dot(rayDirection, rayOrigin - center), 2) - glm::dot(rayDirection, rayDirection)*(glm::dot(rayOrigin - center, rayOrigin - center) - radius * radius);
	...
	if( discriminant >= 0 ) {
		...
		if( discriminant > 0 ) {
			...
			float t1 = (glm::dot(-rayDirection, rayOrigin - center) - sqrt(discriminant)) / glm::dot(rayDirection, rayDirection);
			float t2 = (glm::dot(-rayDirection, rayOrigin - center) + sqrt(discriminant)) / glm::dot(rayDirection, rayDirection);
			...
			t = glm::dot(-rayDirection, rayOrigin - center) / glm::dot(rayDirection, rayDirection);
			if (t < 0) {
				t = FLT_MAX;
			}
		}
