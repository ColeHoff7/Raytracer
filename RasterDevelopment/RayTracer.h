...
class RayTracer
{
	...
	*/
	...
	/**
	* Set the default color to which pixels will be set when a traced ray does not 
	* intersect with an object in the scene.
	* @param defaultColor - color to set pixels to 
	*/
	void setDefaultColor(const color & defaultColor) { this->defaultColor = defaultColor; }

	/**
	* Accessor for the default color. Return color to which pixels are set when a 
	* traced ray does not intersect with an object in the scene.
	* @returns defaultColor - color to set pixels to 
	*/
	color getDefaultColor(const color & defaultColor) { return this->defaultColor; }


	/**
	* Set the maximum depth of the recursion tree for an individual ray.
	* @param recursionDepth - maximum of the recursion tree for each individual ray
	*/
	void setRecursionDepth(const int recursionDepth) { this->recursionDepth = recursionDepth; }


	/**
	* Accessor for maximum recursion depth for individual rays.
	* @returns recursionDepth - maximum of the recursion tree for each individual ray
	*/
	int getRecursionDepth(const color & defaultColor) { return this->recursionDepth; }
	...
	*/
	color traceIndividualRay(const glm::vec3 &e, const glm::vec3 &d, int recursionLevel);
	...
	*/
	...
	// Maximum depth of the recurion tree for an individual ray.
	int recursionDepth;
